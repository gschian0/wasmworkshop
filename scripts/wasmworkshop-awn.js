/* Declares the wasmworkshop Audio Worklet Node */

class wasmworkshopController extends WAMController
{
  constructor (actx, options) {
    ...
    super(actx, "wasmworkshop", options);
  }

  static importScripts (actx) {
    var origin = "https://gschian0.github.io/wasmworkshop/";

    return new Promise( (resolve) => {
      actx.audioWorklet.addModule(origin + "scripts/wasmworkshop-wam.js").then(() => {
      actx.audioWorklet.addModule(origin + "scripts/wam-processor.js").then(() => {
      actx.audioWorklet.addModule(origin + "scripts/wasmworkshop-awp.js").then(() => {
        resolve();
      }) }) });
    })
  }
